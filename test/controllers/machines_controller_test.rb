...
class MachinesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create machine" do
    assert_difference("Machine.count") do
      post machines_url, params: { machine: { enterprise_id: @machine.enterprise_id, name: @machine.name, status: @machine.status, type: @machine.type } }
    end
  ...
  test "should update machine" do
    patch machine_url(@machine), params: { machine: { enterprise_id: @machine.enterprise_id, name: @machine.name, status: @machine.status, type: @machine.type } }
